...
public class CustomisationSet : Stats
{
    ...
    private void Start()
    {
        //Call the load method - Just an Example of PlayerPrefs
        //Load();
    ...
    {
        //loop through variables adding to string and adding \n after each one
        string temp = characterName + "\n"
            + characterClass.ToString() + "\n"
            + characterRace.ToString() + "\n";
        temp += skinIndex + "\n" + hairIndex + "\n" + mouthIndex + "\n" + eyesIndex + "\n" + clothesIndex + "\n" + armourIndex + "\n" + helmIndex + "\n";
        for (int i = 0; i < characterStats.Length; i++)
        {
            temp += (characterStats[i].statValue + characterStats[i].tempStatValue + characterStats[i].levelTempStatValue) + "\n";
    ...
    void SaveCharacter()
    {
        ...
        saveText.CharacterSaveSlot(path, SaveData());
        #endregion
        
        #region Saving Using PlayerPrefs Example
        /*
        //SetInt for skins
    ...
    private void Load()
    {
        //Check to see if the playerpref key is already saved in Registry
        if(PlayerPrefs.HasKey("Character Name"))
        {
            //If it is get the string
            characterName = PlayerPrefs.GetString("characterName");
        }
        else
        {
            //If it isn't there, display a warning
            Debug.LogWarning("There is nothing saved!!");
        }
    }
    */
    #endregion
